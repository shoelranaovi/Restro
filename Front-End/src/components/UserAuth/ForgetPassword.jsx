...
export default function ForgotPass() {
  ...
  if (!siteKey) {
    console.error("ReCAPTCHA site key is missing. Please set VITE_GOOGLE_SITE_KEY in your .env file.");
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = {
      email,
      recaptchaToken, // Include token in formData if your backend expects it
    };
    ...
    dispatch(forgetpass(formData)).then((data) => {
      try {
        ...
        console.error("Error during forgetpass dispatch:", error);
  ...
  const handleResendEmail = async () => {
    ...
    const formData = {
      email,
      recaptchaToken, // Include token if required for resend
    };
